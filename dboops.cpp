...
class DBManager {
    ...
    public:
        ...
        DBManager() {
            ...
            if (mysql_real_connect(conn, "localhost", "root", "root123", "tflmentoringdb", 0, NULL, 0) == NULL) {
        ...
        bool insertTopic(int id,const std::string& title, const std::string& url, int repoid)
        {
            char query[400];
            sprintf(query, "insert into topics values(%d,'%s','%s',%d)", id, title.c_str(), url.c_str(), repoid);
            if(mysql_query(conn,query))
            {
                std::cerr<<"error while inserting";
                return false;
            }
            else
            {
...
int main(){
    ...
    do{
        ...
        case 3: {
            ...
            std::cout << "Enter URL: ";
            std::cin >> url;
            std::cout << "Enter repository id: ";
            std::cin >> repoid;

            if (ptrDbManager->insertTopic(id,title, url,repoid)) {
...
//  g++ .\dboops.cpp -o db.exe -I "C:\Program Files\MySQL\MySQL Server 8.4\include" -L "C:\Program Files\MySQL\MySQL Server 8.4\lib" -l libmysql